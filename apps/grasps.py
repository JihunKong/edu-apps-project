...
def main():
    ...
    # GRASPS 요소 선택 옵션
    grasps_options = {
        "목표(G)": ["직접 입력", "설명한다", "논증한다", "기술한다", "표현한다", "보여준다", "도출한다", "증명한다", "예측한다", "말한다"],
        "역할(R)": ["직접 입력", "배우", "광고 기획자", "미술작가", "사장", "사업가", "후보자", "캐릭터", "만화가", "요리사", "대통령", "유튜버"],
        "대상(A)": ["직접 입력", "학생", "학부모", "지역 주민", "정부 관계자", "기업가", "전문가", "일반 대중"],
        "상황(S)": ["직접 입력", "학교 행사", "지역 문제", "사회 이슈", "환경 문제", "기술 혁신", "문화 교류"],
        "수행(P)": ["직접 입력", "보고서 작성", "프레젠테이션", "토론", "포스터 제작", "동영상 제작", "캠페인 기획"],
        "기준(S)": ["직접 입력", "창의성", "논리성", "실현 가능성", "협업 능력", "의사소통 능력", "문제 해결력"]
    }
    ...
    # 영역 또는 과목 선택
    area_or_subject = st.selectbox("영역 또는 과목 선택", list(curriculum_standards[school_level].keys()))
    
    # 성취기준 선택
    achievement_standard = st.selectbox("성취기준 선택", curriculum_standards[school_level][area_or_subject])
    
    # 추가 교육과정 입력
    additional_curriculum = st.text_area("추가 교육과정 정보 (선택사항)")
    ...
    def grasps_input(label, options):
        selection = st.selectbox(f"{label} 선택 또는 직접 입력", options)
        if selection == "직접 입력":
            return st.text_input(f"{label} 직접 입력")
        return selection
    
    goal = grasps_input("목표(G)", grasps_options["목표(G)"])
    role = grasps_input("역할(R)", grasps_options["역할(R)"])
    audience = grasps_input("대상(A)", grasps_options["대상(A)"])
    situation = grasps_input("상황(S)", grasps_options["상황(S)"])
    performance = grasps_input("수행(P)", grasps_options["수행(P)"])
    
    standards_selection = st.multiselect("기준(S) 선택 (복수 선택 가능) 또는 직접 입력", grasps_options["기준(S)"])
    if "직접 입력" in standards_selection:
        standards = st.text_area("기준(S) 직접 입력")
    else:
        standards = ", ".join(standards_selection) if standards_selection else ""
    ...
    if st.button("프로젝트 수업 설계 생성"):
        ...
        수업 정보:
        - 학교급: {school_level}
        - 영역/과목: {area_or_subject}
        - 성취기준: {achievement_standard}
        - 추가 교육과정 정보: {additional_curriculum}
        ...
        6. 교사의 역할 및 지원 방법
        7. 학생 활동 예시
        8. 평가 방법 및 루브릭 (4단계 평가 루브릭 포함) - 가로는 평가 요소, 세로는 평가 척도로 5x5 표로 나타내세요.
        9. 기대 효과 및 유의사항
